...
async function recalculateCareerStats(playerId) {
    ...
    const stats = m.map(row => {
        ...
        return { runs, balls, fours, sixes, pStatus, isNotOut, isOut, batted, wickets, runsConceded, oversBowled, maidens, wides, noBalls, hundred: runs >= 100 ? 1 : 0, fifty: (runs >= 50 && runs < 100) ? 1 : 0, duck: (runs === 0 && isOut) ? 1 : 0, fourW: wickets === 4 ? 1 : 0, fiveW: wickets >= 5 ? 1 : 0, isHero: !!row.is_hero };
    });
    ...
    console.log({ totalMatches, totalInnings, totalRuns, totalWickets, totalNO, totalBalls, totalBowlOvers, maxScore });
...
recalculateCareerStats(8).then(() => process.exit(0)).catch(err => { console.error(err); process.exit(1); });
