...
// BallRecord now imported from types.ts
    ...
    persist(
        (set, get) => ({
                    ...
                    dbMatches.forEach(dbMatch => {
                        ...
                        if (existingIdx !== -1) {
                            ...
                            if (local.status === 'completed' && dbMatch.status !== 'completed') {
                                ...
                                if (dbMatch.status === 'completed') merged[existingIdx] = dbMatch;
                            }
                        } else {
                            merged.push(dbMatch);
                        }
            ...
            recordPenalty: (side, runs) => {
                ...
                nextInnings.history.push({ 
                    type: 'penalty', 
                    runs: 0, 
                    extraRuns: runs, 
                    isWicket: false,
                    overNumber: Math.floor(nextInnings.totalBalls / 6),
                    ballNumber: (nextInnings.totalBalls % 6) + 1,
                    ballIndex: nextInnings.totalBalls,
                    isLegal: false,
                    timestamp: new Date().toISOString(),
                    commentary: `Penalty: ${runs} runs` 
                });
            ...
            switchStriker: () => set((state) => ({ strikerId: state.nonStrikerId, nonStrikerId: state.strikerId })),
...
// Alias useCricketScorer to useMatchCenter as per user instructions
