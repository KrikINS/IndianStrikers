...
import { Player, ScheduledMatch, Performer, MatchStatus, MatchStage, FullScorecardData, BallRecord, AppUser, SystemCommentary, CommentaryEventType, LivePlayer, LiveBowler, InningsState, MatchScorerState, UnifiedMatchStore } from '../types';
...
// Types LivePlayer, LiveBowler, InningsState, MatchScorerState, UnifiedMatchStore
// are defined in ../types.ts (single source of truth)
...
export const useMatchCenter = create<UnifiedMatchStore>((set, get) => ({
    ...
    recordBall: async (payload) => {
        ...
        const ballRecord: BallRecord = {
            ...
            outPlayerId: outPlayerId || (isWicket ? state.strikerId : undefined),
            batterName: getPlayerName(outPlayerId || (isWicket ? state.strikerId : '') || ''),
            bowlerName: getPlayerName(state.currentBowlerId || ''),
