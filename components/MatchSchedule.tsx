...
const MatchSchedule: React.FC<MatchScheduleProps> = ({ matches, opponents, onAddMatch, onUpdateMatch, userRole }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.tournament && formData.opponent && formData.date) {
      ...
      // We assume onAddMatch is an async wrapper around addMatch service or handles the state update
      // But looking at App.tsx, we pass storageService.addMatch directly or a wrapper that updates state.
      // Ideally this component should await. For now, let's just pass the payload correctly.
      ...
      <div className="grid lg:grid-cols-2 gap-8">
            ...
            upcoming.map(match => (
              <div key={match.id} className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm border-l-4 border-l-blue-500 hover:shadow-md transition-shadow group relative">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 hover:text-blue-800 rounded-lg text-sm font-bold transition-colors flex items-center justify-center gap-2 mt-4"
                >
                  {new Date(match.date).toDateString() === new Date().toDateString() ? (
                    <><Zap size={16} className="fill-blue-700" /> Live Score</>
                  ) : (
        ...
        <div className="space-y-4">
          ...
          {past.length === 0 ? (
            ...
            past.map(match => (
              <div key={match.id} className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group relative">
                {canEdit && (
                  <button
                    onClick={(e) => { e.stopPropagation(); handleEditClick(match); }}
                    className="absolute top-4 right-4 p-2 text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded-full transition-colors"
                    title="Edit Match"
                  >
                    <Edit2 size={16} />
                  </button>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <div className="text-sm font-bold text-slate-600 mt-1">
                      {match.scoreFor || '0/0'} vs {match.scoreAgainst || '0/0'}
                    </div>
                    <div className={`text-xs font-bold mt-2 ${match.result === 'Won' ? 'text-green-600' : match.result === 'Lost' ? 'text-red-500' : 'text-slate-500'}`}>
                      Result: {match.result}
                    </div>
                  </div>
                  <div className="text-right bg-slate-50 px-3 py-2 rounded-lg">
