...
const ScorecardViewModal: React.FC<ScorecardViewModalProps> = ({ match, isOpen, onClose }) => {
    ...
    const renderInnings = (inn: 1 | 2, ref: React.RefObject<HTMLDivElement>) => {
        ...
        return (
            <div ref={ref} className="bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-2xl space-y-8 min-w-[350px]">
                {/* Header Info */}
                <div className="flex justify-between items-start border-b border-slate-800 pb-6">
                    <div>
                        <h2 className="text-3xl font-black text-white graduate tracking-tighter uppercase italic">{battingTeam}</h2>
                        <div className="flex items-center gap-4 mt-2">
                             <div className="flex items-center gap-1.5 text-slate-500 text-[10px] font-black uppercase tracking-widest">
                                <Trophy size={14} className="text-blue-500" /> {match.tournament}
                             </div>
                             <div className="flex items-center gap-1.5 text-slate-500 text-[10px] font-black uppercase tracking-widest">
                                <MapPin size={14} className="text-blue-500" /> {match.groundId}
                             </div>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-4xl font-black text-blue-500 graduate">{data.totalRuns}/{data.totalWickets}</div>
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody>
                                {data.batting.map((b, i) => (
                                    <tr key={i} className={`border-b border-slate-800/50 hover:bg-white/5 transition-colors ${b.outHow === 'Did Not Bat' ? 'opacity-40' : ''}`}>
                                        <td className="py-3 px-2 font-bold text-slate-200">{b.name}</td>
                                        <td className="py-3 px-2 text-xs font-medium text-slate-400 italic">
                                            {b.outHow === 'Not Out' ? <span className="text-emerald-500 italic font-bold">Not Out</span> : b.outHow}
                                        ...
                                        <td className="py-3 px-2 text-right font-black text-white">{b.runs || 0}</td>
                                        <td className="py-3 px-2 text-right text-slate-400">{b.balls || 0}</td>
                                        <td className="py-3 px-2 text-right text-slate-400">{b.fours || 0}</td>
                                        <td className="py-3 px-2 text-right text-slate-400">{b.sixes || 0}</td>
                                        <td className="py-3 px-2 text-right font-mono text-[10px] text-blue-400">{(b.balls || 0) > 0 ? (((b.runs || 0) / b.balls) * 100).toFixed(1) : '-'}</td>
                                    </tr>
                                ))}
                                <tr className="bg-blue-600/5 font-black text-white">
                            ...
                            <tbody>
                                {data.bowling?.filter(b => b.playerId).map((b, i) => (
                                    <tr key={i} className="border-b border-slate-800/50 hover:bg-white/5">
                                        <td className="py-3 px-2 font-bold text-slate-200">{b.name}</td>
                                        <td className="py-3 px-2 text-right font-black text-white">{b.overs || 0}</td>
                                        <td className="py-3 px-2 text-right text-slate-400">{b.maidens || 0}</td>
                                        <td className="py-3 px-2 text-right text-slate-400">{(b as any).runsConceded || (b as any).runs || 0}</td>
                                        <td className="py-3 px-2 text-right font-black text-emerald-400">{b.wickets || 0}</td>
                                        <td className="py-3 px-2 text-right font-mono text-[10px] text-blue-400">
                                            {(() => {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 md:p-8 overflow-y-auto custom-scrollbar">
            <div className="w-full max-w-6xl flex flex-col gap-6">
                
                {/* Fixed Control Bar */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/50 p-6 rounded-3xl border border-white/5 backdrop-blur-xl">
                    <div>
